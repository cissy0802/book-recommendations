...
本仓全站深色（睡前读物）。**生成页面时照常写你觉得合适的浅色配色**——push 之后
`darken.py` 会把内联 `<style>`、`style=""` 和内联 SVG 的 `fill=`/`stroke=` 一起
按亮度推到深色，幂等，已经转过的页面会跳过。

不要为了"配合深色"自己手写死一套黑：那会让每篇文章长得一模一样。
darken.py 保留色相和彩度，原设计的性格还在，只换明暗。

**别在 `<style>` 顶部手写 `/* bigcat-dark v1 */`。** 那是"已转换"的标记，
写上去这一页就永远跳过转换、永远停在浅色。

`darken.py` 顶部的 `TINT_HUE` 是**本站的身份色相**（本仓 72°，暗橄榄金，取自
#c8a868 这套书页金，往冷挪开佛经的暖褐）。原始底色是近白的米色、几乎不带彩度，照原色相推暗会和其他深色站
收敛成同一种黑；所以「接近中性」的面改用这个色相重新上色，真正有颜色的
元素保持自己的颜色。改它等于改本站的整体调性。
